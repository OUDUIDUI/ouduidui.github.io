...
![inset](/images/clipping_and_masking/1.png)
## circle
```css
.clip-path-2 {
...
```
![circle](/images/clipping_and_masking/2.png)
## ellipse
```css
.clip-path-3 {
...
![ellipse](/images/clipping_and_masking/3.png)
## polygon
```css
.clip-path-5 {
...
![polygon](/images/clipping_and_masking/4.png)
...
```
![SVG-clipPath](/images/clipping_and_masking/5.png)

## 利用`clip-path`实现变形动画效果
![clip-path-animation](/images/clipping_and_masking/6.gif)
### 圆形缩放效果
```css
.clip-path-7 {
...
```
![mask1](/images/clipping_and_masking/7.png)
...
```
![mask1](/images/clipping_and_masking/8.png)

目前`mask`属性兼容性较差，大部分可以用前缀识别码实现，`IE`和`EDGE`浏览器不支持。
![兼容性](/images/clipping_and_masking/9.png)
