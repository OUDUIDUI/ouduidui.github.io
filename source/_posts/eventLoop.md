...
- 找宏任务的时候，按照入队的顺序，找到已经可以执行的宏任务。可以执行的宏任务，比如请求接口任务，已经接收到接口数据请求了，比如定时器已经到时可以执行了。如果按顺序排到的任务还不可执行的话，解释器会找到下一个宏任务的。之所以这样设计，是因为假如你的代码先写了`setTimeout(fn1,10000)`，后写了`setTimeout(fn2,1000)`，如果按照入队顺序，`fn1`一定在`fn2`先被执行，但当时间还没到10000ms的时候，`fn1`还不可执行，则解释器应该跳过`fn1`去执行`fn2`，否则的话`fn2`会等到11000ms才被执行。
