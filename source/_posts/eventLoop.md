...
![call_stack_1](/images/eventLoop/call_stack_1.gif)

然后开始执行函数`c`，执行的第一个语句是`console.log('c')`。

因此解释器也会将其放入调用栈中。

![call_stack_2](/images/eventLoop/call_stack_2.gif)

当`console.log('c')`方法执行完后，控制台打印了`'c'`，调用栈就会将其移除。

![call_stack_3](/images/eventLoop/call_stack_3.gif)

接着就是执行`a()`函数。

解释器就将`function a() {}`的执行上下文放入调用栈中。

![call_stack_4](/images/eventLoop/call_stack_4.gif)

紧接着就执行`a()`中的语句——`console.log('a')`。

![call_stack_5](/images/eventLoop/call_stack_5.gif)

当函数`a`执行结束后，调用栈就将执行上下文移除。

然后接着执行`c()`函数剩下的语句，也就是执行`b()`函数，因此它的执行上下文就加入调用栈中。

![call_stack_6](/images/eventLoop/call_stack_6.gif)

紧接着就执行`b()`中的语句——`console.log('b')`。

![call_stack_7](/images/eventLoop/call_stack_7.gif)

`b()`执行完后，调用栈就将其移出。

这时`c()`也执行结束了，调用栈也将其移出栈。

![call_stack_8](/images/eventLoop/call_stack_8.gif)
...
![async](/images/eventLoop/async.png)
...
- 找宏任务的时候，按照入队的顺序，找到已经可以执行的宏任务。可以执行的宏任务，比如请求接口任务，已经接收到接口数据请求了，比如定时器已经到时可以执行了。如果按顺序排到的任务还不可执行的话，解释器会找到下一个宏任务的。之所以这样设计，是因为假如你的代码先写了`setTimeout(fn1,10000)`，后写了`setTimeout(fn2,1000)`，如果按照入队顺序，`fn1`一定在`fn2`前面先被执行，但当时间还没到10000ms的时候，`fn1`还不可执行，则解释器应该跳过`fn1`去执行`fn2`，否则的话`fn2`会等到11000ms才被执行。
...
![task_queque_1](/images/eventLoop/task_queque_1.gif)

按照代码顺序，首先执行`console.log("a")`。

该函数上下文被推入调用栈，执行完后，即移除调用栈。

![task_queque_2](/images/eventLoop/task_queque_2.gif)

接下来执行`setTimeout()`，该函数上下文也进入调用栈中。

![task_queque_3](/images/eventLoop/task_queque_3.gif)

因为`setTimeout`是一个宏任务，因此将其`callback`函数推入宏任务队列中，然后该函数就被移除调用栈，继续往下执行。

![task_queque_4](/images/eventLoop/task_queque_4.gif)

紧接着是`Promise`语句，先将其放入调用栈，然后接着往下执行。

![task_queque_5](/images/eventLoop/task_queque_5.gif)

执行`console.log("c")`和`resolve()`，这里就不多说了。

![task_queque_6](/images/eventLoop/task_queque_6.gif)

接着来到`new Promise().then()`方法，这是一个微任务，因此将其推入微任务队列中。

![task_queque_7](/images/eventLoop/task_queque_7.gif)

这时`new Promise`语句已经执行结束了，就被移除调用栈。

接着做执行`console.log('f')`。

![task_queque_8](/images/eventLoop/task_queque_8.gif)

这时候，`script`宏任务已经执行结束了，因此被推出宏任务队列。

紧接着开始清空微任务队列了。首先执行的是`Promise then`，因此它被推入调用栈中。

![task_queque_9](/images/eventLoop/task_queque_9.gif)

然后开始执行其中的`console.log("d")`。

![task_queque_10](/images/eventLoop/task_queque_10.gif)

执行结束后，检测到后面还有一个`then()`函数，因此将其推入微任务队列中。

此时第一个`then()`函数已经执行结束了，就会移除调用栈和微任务队列。

![task_queque_11](/images/eventLoop/task_queque_11.gif)

此时微任务队列还没被清空，因此继续执行下一个微任务。

执行过程跟前面差不多，就不多说了。

![task_queque_12](/images/eventLoop/task_queque_12.gif)

此时微任务队列已经清空了，第一个事件循环已经结束了。

接下来执行下一个宏任务，即`setTimeout callback`。

![task_queque_13](/images/eventLoop/task_queque_13.gif)
...
![task_queque_14](/images/eventLoop/task_queque_14.gif)
...
![async_await_1](/images/eventLoop/async_await_1.gif)

紧接着是`setTimeout`，将其回调放入宏任务中，然后继续执行。

![async_await_2](/images/eventLoop/async_await_2.gif)

紧接着是调用`async1()`函数，因此将其函数上下文放置到调用栈。

![async_await_3](/images/eventLoop/async_await_3.gif)

然后开始执行`async1`中的`console.log("a")`。

![async_await_4](/images/eventLoop/async_await_4.gif)

接下来就是`await`关键字语句。

`await`后面调用的是`async2`函数，因此我们将其放入调用栈。

![async_await_5](/images/eventLoop/async_await_5.gif)

然后开始执行`async2`中的`console.log("c")`，并`return`一个值。

执行完成后，`async2`就被移出调用栈。

![async_await_6](/images/eventLoop/async_await_6.gif)

这时候，`await`会阻塞`async2`的返回值，先跳出`async1`进行往下执行。

需要注意的是，现在`async1`中的`res`变量，还是`undefined`，没有赋值。

![async_await_7](/images/eventLoop/async_await_7.gif)

紧接着是执行`new Promise`。

![async_await_8](/images/eventLoop/async_await_8.gif)

执行`console.log("i")`。

![async_await_9](/images/eventLoop/async_await_9.gif)

这时，`async1`外面的同步任务都执行完成了，因此就重新回到前面阻塞的位置，进行往下执行。

![async_await_10](/images/eventLoop/async_await_10.gif)

这时`res`成功赋值了`async2`的结果值，然后往下执行`console.log("b")`。

![async_await_11](/images/eventLoop/async_await_11.gif)

这时候`async1`才算是执行结束，紧接着再将其调用的`then()`函数放入微任务队列中。

![async_await_12](/images/eventLoop/async_await_12.gif)

这时`script`宏任务已经全部执行完了，开始准备清空微任务队列了。

第一个被执行的微任务队列是`promise then`，也就是将执行其中的`console.log("h")`语句。

![async_await_13](/images/eventLoop/async_await_13.gif)

执行完`Promise then`微任务后，紧接着开始执行`async1`的`promise then`微任务。

![async_await_14](/images/eventLoop/async_await_14.gif)

这时候微任务队列已经清空了，即开始执行下一个宏任务。

![async_await_15](/images/eventLoop/async_await_15.gif)
...
![render_1](/images/eventLoop/render_1.gif)

第一个被执行的是`console.log("a")`。

![render_2](/images/eventLoop/render_2.gif)

其次是`setTimeout`，并将其回调加入宏任务队列中。

![render_3](/images/eventLoop/render_3.gif)

紧接着执行`new Promise`。

![render_4](/images/eventLoop/render_4.gif)

同样，将其`then()`推入微任务队列中去。

![render_5](/images/eventLoop/render_5.gif)

紧接着执行`console.log("e")`。

![render_6](/images/eventLoop/render_6.gif)

最后，修改DOM节点的文本内容，但是这时候页面还不会更新渲染。

这时候`script`宏任务也执行结束了。

![render_7](/images/eventLoop/render_7.gif)

紧接着，开始清空微任务队列，执行`Promise then`。

![render_8](/images/eventLoop/render_8.gif)

这时候，`alert`一个通知，而这个语句结束后，则微任务队列清空，代表第一个事件循环结束，即将要开始渲染页面了。

![render_9](/images/eventLoop/render_9.gif)

当点击关闭`alert`后，事件循环结束，页面也开始渲染。

![render_10](/images/eventLoop/render_10.gif)

渲染结束后，就开始执行下一个宏任务，即`setTimeout callback`。

![render_11](/images/eventLoop/render_11.gif)

紧接着执行`console.log("b")`。

![render_12](/images/eventLoop/render_12.gif)

这时候宏任务队列已清空了，但是`html`文件还没执行结束，因此进入`render2.js`继续执行。

![render_13](/images/eventLoop/render_13.gif)

首先执行`console.log('f')`。

![render_14](/images/eventLoop/render_14.gif)

紧接着，再次修改节点的文本信息，此时依旧不会更新页面渲染。

接着执行`alert`语句，当关闭`alert`通知后，该宏任务结束，微任务队列也为空，因此该事件循环也结束了，这时候就开始第二次页面更新。

![render_15](/images/eventLoop/render_15.gif)
...
```
输出：a c e d "开始渲染" f "第二次渲染" b "渲染结束"
```
